...
/**
 * Procedural Industrial Servo Actuator 3D Model with Independent Component Explosion Trajectories
 */
function ActuatorModel({ scrollProgress = 0, isHovered, onHoverChange, onClickActuator }) {
  ...
  useFrame((state, delta) => {
    ...
    // Subtle gentle slow rotation of the overall actuator assembly
    if (groupRef.current) {
      groupRef.current.rotation.y = -0.35 + Math.sin(state.clock.elapsedTime * 0.4) * 0.06;
    ...
    if (r >= 0.75 && pistonRef.current) {
      const demoExt = Math.sin(state.clock.elapsedTime * 1.8) * 0.22;
      pistonRef.current.position.z = 0.8 + demoExt;
    } else if (pistonRef.current) {
      // Physical assembly movement based on scroll ratio
  ...
  return (
    ...
    >
      ...
      {/* 02. SERVO MOTOR HOUSING (z = -1.3 -> 0) */}
...
function CameraController({ scrollProgress }) {
  useFrame((state, delta) => {
    // Cinematic camera pathing driven by scroll ratio
    // Stage 01 (0.0-0.25): Wide exploded overview
    // Stage 02 (0.25-0.50): Close-up of internal ball screw & bearings
    // Stage 03 (0.50-0.75): Transition orbit as housing & motor lock
    // Stage 04 (0.75-1.00): Hero presentation view
    const r = scrollProgress;

    let targetCamX = 4.8 - r * 1.2;
    let targetCamY = 2.8 - r * 1.2;
    let targetCamZ = 5.2 - r * 1.2;
...
export default function ActuatorSceneCanvas({ scrollProgress = 0, onClickActuator }) {
  const [isHovered, setIsHovered] = useState(false);

  return (
    <div className={`actuator-3d-canvas-layer ${scrollProgress >= 0.75 ? 'assembled-active' : ''}`}>
      <Canvas
