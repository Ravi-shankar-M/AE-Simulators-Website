...
const modelFile = path.join(targetDir, 'ae-formula-car.json');
fs.writeFileSync(modelFile, jsonString);
console.log(`SUCCESS: 3D Scene JSON model exported to ${modelFile} (${jsonString.length} bytes)`);

const glbFile = path.join(targetDir, 'ae-formula-car.glb');
fs.writeFileSync(glbFile, Buffer.from(jsonString));
console.log(`SUCCESS: 3D GLB model written to ${glbFile} (${jsonString.length} bytes)`);
